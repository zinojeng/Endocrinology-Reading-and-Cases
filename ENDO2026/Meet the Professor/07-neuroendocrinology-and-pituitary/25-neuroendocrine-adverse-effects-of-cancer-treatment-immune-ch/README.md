...
> 📊 表 1：ICI 相關 hypophysitis 的臨床特徵（原書表格）
> **Table 1. Clinical Features of ICI-Associated Hypophysitis**
...
> 📊 圖 1：ICI 治療開始後至 hypophysitis 診斷的時間分布（原書圖，取自 Faje A et al. *Eur J Endocrinol* 2019；181(3):211-219）
...
> 📊 圖 2：Ipilimumab 引起 hypophysitis 病人的 MRI 變化序列（原書圖，取自 Chamorro-Pareja N et al. *Endocrinology* 2024；165(9):bqae084）
> A：ipilimumab 開始前 2 天；B：診斷前 4 天；C：hypophysitis 診斷時；D：診斷後 4 天；E：診斷後 6 週；F：診斷後 9 週；G：診斷後 15 週；H：診斷後 19 週。
...
> 📊 圖 3：Case 1 的 MRI 與 FDG-PET（原書圖）。A：ICI 治療前 MRI；B：第 2 cycle 後 3 個月 MRI；C：第 2 cycle 後 1 週 FDG-PET。
...
> 📊 圖 4：Case 2 的 MRI（原書圖）。A：ICI 治療前；B：住院期間（可見瀰漫性腺體腫大）。
...
> 📊 圖 5：Case 3 的 MRI（原書圖）。A：ICI 治療前；B：presentation 時。
